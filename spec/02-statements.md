...
- **Next gate.** Implementation in `internal/lexer/`, `internal/parser/`, and the print/write/read evaluators is unlocked once `spec/01-lexical-structure.md`, `spec/02-statements.md`, and `spec/03-rules.md` are all harmonized with Decision 6. As of this pass, all three specs are consistent. The pre-implementation parser hazard remains tracked in `tracking/issues/14-parser-silent-token-drop.md`.