...
| `V<phase>`   | Current Manifest Phase/version (e.g. `8.7`)                    | `MANIFEST.md` |
| `D<epoch>`   | Highest authored decision id (e.g. `15`)                       | `todo/DECISIONS.md` |
| `<hash8>`    | First 8 hex of SHA-256 over the canonical context files        | see below |

**Current value:** `BEE-LANG-V8.7-D15-c58f3dc8`
...
- `scripts/context_signature.py` — deterministic generator (single source of
  truth for the hash).
- `.agents/skills/bee/SKILL.md` §4 — instructs the agent to compute and
  prepend the signature at session start.
- This file (`CONTEXT.md`) — the human-readable specification and the record
...
- The signature relies on `todo/DECISIONS.md` being internally consistent. A
  stale/contradictory decision index (e.g. D10/D11 status drift, missing D15)
  weakens the human-readable `D<epoch>` but not the hash — fix the index
  separately (see `todo/TUTORIAL_TODO.md` §3.1).
