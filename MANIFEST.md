...
- **Decision 9 – Colon `:` is the pair-up operator (✅ ratified 2026-09-13)**: `:` is the canonical structural binding operator — key-to-value, parameter-to-type, identifier-to-literal, label-to-block-head — **without** type inference, ever. `=` is value equality (logic), `:=` is type-inferred assignment, `:` is structural pair-up and **requires** an explicit type context: in declarations the trailing `∈ Type` is mandatory, so `new a: 1, b: 2 ∈ Z;` is legal while bare `new a: 1;` is E0009 (use `:=` for inference). Cross-context grammar extension enables the parallel typed declaration (Issue 18). See `todo/DECISIONS.md` D9.
